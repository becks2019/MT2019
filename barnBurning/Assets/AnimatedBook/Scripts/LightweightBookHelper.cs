...
namespace com.guinealion.animatedBook {
	...
	public class LightweightBookHelper : MonoBehaviour {
		...
		[SerializeField] private Transform m_frontCover;
		[SerializeField] private Transform m_backCover;
		[SerializeField] private Transform m_boneRoot;
		...
		public float OpenAmmount {
			...
			set {
				...
				float lerp = (-_orientation + 1) * .5f;
				float start = _orientation * -90;

				m_boneRoot.localPosition = closedOffset * (1 - Mathf.Abs(Orientation)) * (1 - value);

				m_backCover.localEulerAngles = new Vector3(0, 0,start - 180 * value * lerp);
				m_frontCover.localEulerAngles = new Vector3(0, 0,start + 180 * value * (1-lerp));
